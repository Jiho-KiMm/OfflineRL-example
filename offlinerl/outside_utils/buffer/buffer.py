...
class ReplayBuffer:
    ...
    def load_dataset(self, dataset: Dict[str, np.ndarray]) -> None:
        observations = np.array(dataset["obs"], dtype=self.obs_dtype)
        next_observations = np.array(dataset["obs_next"], dtype=self.obs_dtype)
        actions = np.array(dataset["act"], dtype=self.action_dtype)
        rewards = np.array(dataset["rew"], dtype=np.float32).reshape(-1, 1)
        terminals = np.array(dataset["done"], dtype=np.float32).reshape(-1, 1)
    ...
    def normalize_obs(self, eps: float = 1e-3, inplace : bool = True) -> Tuple[np.ndarray, np.ndarray]:
