...
class AlgoTrainer(BaseAlgo):
    ...
    def _train_transition(self, transition, data, optim):
        ...
        if len(data['rew'].shape) != len(data['obs_next'].shape) :
            loss = - dist.log_prob(torch.cat([data['obs_next'], data['rew'].unsqueeze(-1)], dim=-1))
        else:
    ...
    def _eval_transition(self, transition, valdata):
        with torch.no_grad():
            ...
            if len(valdata['rew'].shape) != len(valdata['obs_next'].shape):
                loss = ((dist.mean - torch.cat([valdata['obs_next'], valdata['rew'].unsqueeze(-1)], dim=-1)) ** 2).mean(dim=(1,2))
            else:
